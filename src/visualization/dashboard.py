...
with st.sidebar:
    st.markdown("### ♟ Filters")
    st.markdown("---")
    elo_range = st.slider("Elo range", elo_min, elo_max,
                          (elo_min, elo_max), step=50)
...
# ── Apply filters ─────────────────────────────────────────────────────────────
mask = (
    (df["white_elo"] >= elo_range[0]) & (df["white_elo"] <= elo_range[1]) &
    (df["black_elo"] >= elo_range[0]) & (df["black_elo"] <= elo_range[1]) &
...
with col_l:
    ...
    dff2["elo_bin"] = pd.cut(
        ...
        labels=["Black ≫ White", "Black › White", "Even",
                "White › Black", "White ≫ Black"],
    )
    grp    = dff2.groupby(["elo_bin", "outcome"], observed=True).size().reset_index(name="n")
    totals = grp.groupby("elo_bin", observed=True)["n"].transform("sum")
    grp["pct"] = (grp["n"] / totals * 100).round(1)

    bin_labels = ["Black ≫ White", "Black › White", "Even",
                  "White › Black", "White ≫ Black"]
    html_rows  = ""
...
with col_r:
    ...
    if len(dff_sf) > 30:
        ...
        for oc, label in label_map.items():
            ...
            if len(vals) < 5:
                continue
            s = dict(q1=vals.quantile(0.25), q3=vals.quantile(0.75),
                     med=vals.median(),
                     lo=vals.quantile(0.05), hi=vals.quantile(0.95))
            color = color_map[oc]
        ...
        st.info("Not enough Stockfish games for this filter. "
...
with col_b:
    ...
    fig_elo = go.Figure()
    for col_name, color, name in [("white_elo", WHT_COL, "White Elo"),
...
with col_c:
    ...
    fig_eco = go.Figure()
    for oc, color, name in [("white", WHT_COL, "White"),
                             ("draw",  DRW_COL, "Draw"),
                             ("black", BLK_COL, "Black")]:
        fig_eco.add_trace(go.Bar(
            y=eco_df["family"], x=eco_df[oc],
    ...
    fig_eco.update_layout(
        ...
        xaxis=dict(gridcolor=BORDER, ticksuffix="%", zerolinecolor=BORDER,
                   tickfont=dict(family="DM Mono", size=10)),
...
with col_d:
    ...
    if err_cols.issubset(dff.columns):
        err_rows = []
        for oc, label in [("white","White wins"),("draw","Draw"),("black","Black wins")]:
        ...
        for col_name, color, name in [
            ...
            fig_err.add_trace(go.Bar(
                x=err_df["outcome"], y=err_df[col_name].round(2),
                name=name, marker_color=color, marker_line_width=0,
                hovertemplate=f"<b>{name}</b><br>%{{x}}: %{{y:.2f}} avg<extra></extra>",
            ))
        fig_err.update_layout(
            **PLOTLY_BASE, barmode="group", bargap=0.2, bargroupgap=0.05,
            legend=dict(orientation="h", y=-0.28, x=0,
                        font=dict(family="DM Mono", size=10), bgcolor="rgba(0,0,0,0)"),
            height=300,
        )
    ...
    if len(dff_sf) > 50:
        ...
        for oc, color in [("white", WHT_COL), ("draw", DRW_COL), ("black", BLK_COL)]:
            ...
            fig_sc.add_trace(go.Scatter(
                x=sub["elo_gap"], y=sub["acl_gap"],
                mode="markers", name=oc.capitalize(),
                marker=dict(color=color, size=3, opacity=0.45),
                hovertemplate=(f"<b>{oc.capitalize()}</b><br>"
...
band_grp = (dff4.groupby(["elo_band", "outcome"], observed=True)
...
fig_trend.update_layout(
    ...
    yaxis=dict(gridcolor=BORDER, ticksuffix="%", zerolinecolor=BORDER,
               tickfont=dict(family="DM Mono", size=10), range=[0, 70]),
